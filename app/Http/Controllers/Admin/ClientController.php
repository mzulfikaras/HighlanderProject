...
class ClientController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $dataClient['image'] = $request->file('image')->store('assets/gallery', 'public');
        $client = Client::create($dataClient);
        $request->session()->flash('pesan', "Client {$dataClient['nama_client']} Berhasil diSimpan");
    ...
    {
        $dataClient = $request->validate([
            'nama_client' => 'required'.$client->id,
            'image' => 'required|file|image|max:4000',
            'description' => 'required',
        ]);
        if($request->image){
            Storage::delete('public/'.$dataClient->image);
            $dataClient['image'] = $request->file('image')->store('assets/img', 'public');
        }
